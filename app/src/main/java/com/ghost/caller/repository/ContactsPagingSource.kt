...
class ContactsPagingSource(
                ...
                while (it.moveToNext()) {
                    ...
                    contacts.add(
                        ContactQuickInfo(
                            ...
                            primaryPhoneNumber = null, // Lazy load when needed
                            primaryEmail = null,
                            photoUri = photoUri,
    ...
    private fun buildSelection(filter: ContactFilter): String? {
        return when (filter) {
            ContactFilter.STARRED -> "${ContactsContract.Contacts.STARRED} = 1"
            else -> null
        }
    }
    ...
    private fun buildSortOrder(sortBy: ContactSort): String {
        return when (sortBy) {
            ContactSort.NAME_ASC -> "${ContactsContract.Contacts.DISPLAY_NAME} COLLATE LOCALIZED ASC"
            ContactSort.NAME_DESC -> "${ContactsContract.Contacts.DISPLAY_NAME} COLLATE LOCALIZED DESC"
            ContactSort.RECENT_ASC -> "${ContactsContract.Contacts.LAST_TIME_CONTACTED} ASC"
            ContactSort.RECENT_DESC -> "${ContactsContract.Contacts.LAST_TIME_CONTACTED} DESC"
        }
    }
